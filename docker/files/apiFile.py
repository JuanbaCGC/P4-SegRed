...
def get(username, doc_id):
    ...
    respuesta = requests.get(f'https://10.0.2.3:5000/{username}/verify', headers=header, verify='/usr/local/share/ca-certificates/dockerAuth.crt')
...
def post(username,doc_id):
    ...
    # Verificamos la cabecera y el token
    respuesta = requests.get(f'https://10.0.2.3:5000/{username}/verify', json=parameters, headers=header, verify='/usr/local/share/ca-certificates/dockerAuth.crt')
    respuesta_json = respuesta.json()
    if 'Correct' in respuesta_json:
        documents_list = os.listdir(root+"/"+username)
...
if __name__ == '__main__':
    ...
    app.run(host="0.0.0.0", ssl_context=("filescert.pem", "fileskey.pem"), port=5000)
